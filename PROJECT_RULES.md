...
### 版本号更新强制检查
- **强制要求**：**每次代码修改后都必须检查并更新版本号**，无例外！
- **检查时机**：在调用 `done` 工具之前**必须**执行以下检查：
  1. 读取当前 `version.json` 文件
  2. 判断修改类型：
     - 新增功能/重大架构调整 → 升级次版本号（如 0.4.0 → 0.5.0）
     - Bug 修复/UI 调整/性能优化 → 升级修订号（如 0.4.0 → 0.4.1）
  3. 更新 `version.json` 文件中的版本号和历史记录
  4. 验证 API `/api/v1/version` 返回正确的版本号
- **禁止行为**：调用 `done` 工具前忘记更新版本号
- **自动化建议**：在 `write_todos` 中始终包含"检查并更新版本号"步骤
...
- **版本号格式**：`0.x.x`
- **升级规则**：
  - **主版本号（0）**：项目处于开发阶段，尚未正式发布（禁止修改）
  - **次版本号**：新增主要功能模块、重大架构调整
  - **修订号**：Bug 修复、UI 调整、性能优化
...
在每次任务完成前，**必须**回答以下问题：
- [ ] 本次修改是否产生了任何代码变更？
- [ ] 修改类型是：新增功能 / Bug 修复 / 其他？
- [ ] 已正确更新 `version.json` 文件？
- [ ] API `/api/v1/version` 返回新版本号？
- [ ] 版本号符合语义化版本规范？
- **全部打钩后才能调用 `done` 工具！**
...
### 自测规范 [CRITICAL]
- **强制要求**：**所有功能修改完成后，必须使用测试数据进行完整自测**
- **自测时机**：在调用 `done` 工具之前**必须**完成自测
- **自测内容**：
  1. 创建测试数据（目标、任务、待办等）
  2. 验证创建功能：创建后数据能正确显示在列表中
  3. 验证编辑功能：修改后数据能正确更新
  4. 验证删除功能：删除后数据能正确移除
  5. 验证页面导航：创建/编辑后能正确跳转或返回
- **禁止行为**：修改代码后不做自测直接交付
- **更新时间**：2026-02-11
...
- **2026-02-11**：强化版本管理规范，明确每次修改都必须检查并更新版本号，添加强制检查清单
